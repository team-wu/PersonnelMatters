	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try{
			...
			if(method.equals("one")){//用户登录
				...
				if(sf.equals("用户")){
					String str=cBean.getString("select id from employee where name='"+username+"' and  password='"+password+"'  ");
					if(str==null){
				...
				if(sf.equals("用户")){
					String str=cBean.getString("select id from employee where number='"+username+"' and  password='"+oldpwd+"'");
					if(str==null){
						request.setAttribute("message", "原始密码信息错误！");
						request.getRequestDispatcher("admin/system/editpwd.jsp").forward(request, response); 
					}
					else{
						int flag=cBean.comUp("update employee set password='"+newpwd+"' where number='"+username+"'");
