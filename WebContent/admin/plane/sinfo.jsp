...
function top2(){
   	form3.action="<%=basePath%>admin/plane/sinfo.jsp?page=1";
    form3.submit();
}
function last2(){
	...
	form3.action="<%=basePath%>admin/plane/sinfo.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre2(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>admin/plane/sinfo.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump2(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%>admin/plane/sinfo.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%>admin/plane/sinfo.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%>admin/plane/sinfo.jsp?page="+page;
      form3.submit();
...
<div class="title_right"><strong>工作计划管理</strong></div>  
...
<table class="table table-bordered table-striped table-hover">
     <tbody>
       <tr align="center">
         ...
         <td nowrap="nowrap"><strong>员工编号</strong></td> 
         <td nowrap="nowrap"><strong>任务内容</strong></td>
         <td nowrap="nowrap"><strong>完成期限</strong></td>
         <td nowrap="nowrap"><strong>发布时间</strong></td>
         <td nowrap="nowrap"><strong>姓名</strong></td>  
         <td nowrap="nowrap"><strong>状态</strong></td>  
         <td nowrap="nowrap"><strong>高级用户</strong></td>  
         <td width="80" nowrap="nowrap"><strong>操作</strong></td>
       </tr>
<%String word=Common.toChineseAndTrim(request.getParameter("word"));  
	cb.setEVERYPAGENUM(12);
	int cou = cb.getMessageCount("select count(*) from plane where number like '%"+word+"%' and vip='"+username+"'");//得到信息总数			        
	...
	List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from plane where number like '%"+word+"%' and vip='"+username+"' order by id desc",8);
...
%>       
       <tr align="center">
         ...
         <td nowrap="nowrap"> 
         <a href="<%=basePath%>admin/plane/add.jsp?method=upjh&id=<%=pagelist2.get(0).toString()%>">修改</a> 
         <a href="<%=basePath%>ComServlet?method=deljh&id=<%=pagelist2.get(0).toString()%>">删除</a>
         </td>
       </tr>
<% }} %>
       <tr align="center">
