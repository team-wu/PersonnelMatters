...
String message = (String)request.getAttribute("message");
	...
	if(username==null){
		...
		if(method.equals("upjh")){
			id=request.getParameter("id");
			List jlist = cb.get1Com("select * from plane where id='"+id+"'",4);
...
<table class="table table-bordered"> 
    ...
    <%if(method.equals("upjh")){ %><option value="<%=number%>"><%=number%></option> <%} %> 
    ...
    <%}} %>
     ...
     <tr>
     <td width="40%" align="right" nowrap="nowrap" bgcolor="#f1f1f1">任务内容：</td>
     <td><textarea name="workContent" class="span4" rows="5" required><%=workContent %></textarea></td> 
     </tr> 
     <tr>
     <td width="40%" align="right" nowrap="nowrap" bgcolor="#f1f1f1">完成期限：</td>
     <td><input type="date" name="term" class="span2" value="<%=term %>" required/></td> 
     </tr> 
