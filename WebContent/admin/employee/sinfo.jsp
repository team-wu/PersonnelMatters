...
function top2(){
   	form3.action="<%=basePath%>admin/employee/sinfo.jsp?page=1";
    form3.submit();
}
function last2(){
	...
	form3.action="<%=basePath%>admin/employee/sinfo.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre2(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>admin/employee/sinfo.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump2(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%>admin/employee/sinfo.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%>admin/employee/sinfo.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%>admin/employee/sinfo.jsp?page="+page;
      form3.submit();
...
<table class="table table-bordered table-striped table-hover">
     <tbody>
       <tr align="center">
         ...
         <td nowrap="nowrap"><strong>员工工号</strong></td> 
         <td nowrap="nowrap"><strong>员工姓名</strong></td>
         <td nowrap="nowrap"><strong>所属部门</strong></td>
         <td nowrap="nowrap"><strong>员工年龄</strong></td>
         <td nowrap="nowrap"><strong>员工性别</strong></td> 
         <td nowrap="nowrap"><strong>学历信息</strong></td>  
         <td nowrap="nowrap"><strong>专业信息</strong></td>  
         <td nowrap="nowrap"><strong>职务信息</strong></td>  
         <td nowrap="nowrap"><strong>移动电话</strong></td>   
...
<%String word=Common.toChineseAndTrim(request.getParameter("word"));  
	...
	int cou = cb.getMessageCount("select count(*) from employee where number like '%"+word+"%'  ");//得到信息总数			        
	...
	List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from employee where number like '%"+word+"%'  order by id desc",13);
...
%>       
       <tr align="center">
         ...
         <td nowrap="nowrap"> 
         <a href="<%=basePath%>admin/employee/add.jsp?method=upemployee&id=<%=pagelist2.get(0).toString()%>">修改</a> 
         <a href="<%=basePath%>ComServlet?method=delemployee&id=<%=pagelist2.get(0).toString()%>">删除</a>
         </td>
       </tr>
<% }} %>
       <tr align="center">
